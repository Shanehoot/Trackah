    ...
    def add_template(self, name, food_data):
        ...
        if self.use_firestore:
            self.db.collection('templates').add({
                'name': name, 'food_items_json': data_str,
                'total_calories': food_data.get('calories', 0), 'total_protein': food_data.get('protein', 0),
                'uid': unique_id
            })
        else:
            conn = sqlite3.connect(self.sqlite_db)
            conn.execute("INSERT INTO templates (name, food_items_json, total_calories, total_protein, uid) VALUES (?, ?, ?, ?, ?)", (name, data_str, food_data.get('calories', 0), food_data.get('protein', 0), unique_id))
            conn.commit()
            conn.close()
...
def analyze_food_with_gemini(food_input, api_key):
    ...
    Input: "{food_input}"
    CRITICAL:
    1. If input contains MULTIPLE distinct items, calculate nutritional content for ALL items combined.
    2. For 'food_name', create a summary title (e.g. "Potatoes, Carrots & Broccoli").
    3. For 'breakdown', provide a concise string detailing macros for each component (e.g., "5 Potatoes: 500kcal, 10g P; 1 Carrot: 30kcal").
    Return JSON:
    {{
...
def analyze_image_with_gemini(image_bytes, api_key):
    ...
    {{
        ...
        "total_fats": int, "saturated_fat": int, "sodium": int,
        "vitamin_a": int, "vitamin_c": int, "vitamin_d": int, "calcium": int, "iron": int, "potassium": int, "magnesium": int, "zinc": int,
        "breakdown": "string detailing ingredients/methods", "confidence_score": int
    }}
    """
    try:
